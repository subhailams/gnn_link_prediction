...
class LinkTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.attention_weights = nn.Linear(self.out_dim * 3, 3)
    ...
    def apply_attention(self, pairwise_features, shapes):
        """
        Apply attention to the multimodal pairwise features.

        Args:
            pairwise_features: Concatenated graph, visual, and alignment features.
            shapes: A list of shapes for graph, visual, and alignment features.

        Returns:
            Tensor with attention-weighted features.
        """
        # Split features into graph, visual, and alignment components
        graph_features, visual_features, alignment_features = torch.split(
            pairwise_features, shapes, dim=-1
        )

        # Normalize the features (using LayerNorm or L2 normalization)
        graph_features = F.normalize(graph_features, p=2, dim=-1)  # L2 normalization
        visual_features = F.normalize(visual_features, p=2, dim=-1)  # L2 normalization
        alignment_features = F.normalize(alignment_features, p=2, dim=-1)  # L2 normalization

        # Compute attention scores
        self.attention_weights = nn.Linear(sum(shapes), 3)  # 3 attention scores: Graph, Visual, Alignment
        attention_scores = F.softmax(self.attention_weights(pairwise_features), dim=-1)

        # Reshape attention scores for broadcasting
        graph_attention = attention_scores[:, 0].unsqueeze(1).expand(-1, graph_features.size(1))
        visual_attention = attention_scores[:, 1].unsqueeze(1).expand(-1, visual_features.size(1))
        alignment_attention = attention_scores[:, 2].unsqueeze(1).expand(-1, alignment_features.size(1))

        # Multiply attention scores with corresponding features
        graph_weighted = graph_attention * graph_features
        visual_weighted = visual_attention * visual_features
        alignment_weighted = alignment_attention * alignment_features

        # Concatenate the weighted features
        fused_features = torch.cat([graph_weighted, visual_weighted, alignment_weighted], dim=-1)

        return fused_features

        
    def forward(self, batch, adj_prop=None, adj_mask=None, test_set=False, return_weights=False, visual_features=None):
        """
        Calculate edge representations with cross-modality interaction encoding.

        Parameters:
        ----------
            batch: torch.Tensor
                2 x BS Tensor that holds source and target nodes.
            adj_prop: SparseTensor
                Adjacency propagation matrix for graph propagation.
            adj_mask: SparseTensor
                Adjacency mask for masking edges.
            test_set: bool
                Whether evaluating on the test set. Needed if using val_edges in agg.
            return_weights: bool
                Whether to return attention weights.
            visual_features: torch.Tensor
                CLIP visual embeddings for cross-modality interaction (defaults to data['x']).

        Returns:
        --------
            torch.Tensor
                Tensor of shape (batch_size, self.out_dim)
        """
        batch = batch.to(self.device)

        # Step 1: Perform graph propagation
        X_node = self.propagate(adj_prop, test_set)
        x_i, x_j = X_node[batch[0]], X_node[batch[1]]

        # Step 2: Compute elementwise edge features
        elementwise_edge_feats = self.elementwise_lin(x_i * x_j)

        # Step 3: Compute pairwise graph features
        pairwise_feats, att_weights = self.calc_pairwise(
            batch, X_node, test_set, adj_mask=adj_mask, return_weights=return_weights
        )

        # Step 4: Compute cross-modality interaction encoding using CLIP embeddings as visual features
        if visual_features is None:
            visual_features = self.clip_embeddings  # Assume data['x'] has already been assigned to self.clip_embeddings

        # Cross-modality encoding
        self.pairwise_features, shapes = self.compute_cross_modality_encoding(
            X_node, visual_features, batch
        )

        # Step 5: Apply attention to weigh graph, visual, and alignment features
        attention_weighted_feats = self.apply_attention(self.pairwise_features, shapes)

        # Step 6: Combine all features (elementwise, pairwise, and attention-weighted multimodal)
        combined_feats = torch.cat([elementwise_edge_feats, pairwise_feats, attention_weighted_feats], dim=-1)

        # Step 7: Project the combined features to the output dimension
        final_projection_dim = combined_feats.shape[-1]
        self.attention_merge = nn.Linear(final_projection_dim, self.out_dim)
        combined_feats = self.attention_merge(combined_feats)

        # Ensure the output shape matches (batch_size, self.out_dim)
        assert combined_feats.shape[-1] == self.out_dim, \
            f"Output shape mismatch: Expected (-1, {self.out_dim}), got {combined_feats.shape}"
    ...
    def compute_cross_modality_encoding(self, graph_features, visual_features, batch):
        """
        Compute Cross-Modality Interaction Encoding.

        Args:
            graph_features: Graph-based node embeddings.
            visual_features: CLIP visual embeddings.
            batch: Edge indices (2 x BS Tensor holding source and target nodes).

        Returns:
            Tensor with cross-modality pairwise encoding.
        """
        src, dst = batch  # Source and destination nodes

        # Graph pairwise features (element-wise product)
        graph_pairwise = graph_features[src] * graph_features[dst]

        # Visual pairwise features (element-wise product)
        visual_src = visual_features[src]
        visual_dst = visual_features[dst]
        visual_pairwise = visual_src * visual_dst

        alignment = F.cosine_similarity(visual_src, visual_dst, dim=-1).unsqueeze(-1)
        # print("visual_pairwise shape:",visual_pairwise.shape)
        # print("graph_pairwise shape:",graph_pairwise.shape)
        # print("Alignment shape:",alignment.shape)
        shapes = [visual_pairwise.shape[1], graph_pairwise.shape[1], alignment.shape[1]]
        # alignment = torch.nn.Linear(visual_pairwise.shape[0], 256)(alignment)  # Linear layer to project into 256-dimensional space

        # Concatenate graph, visual, and alignment features
        pairwise_features = torch.cat([graph_pairwise, visual_pairwise, alignment], dim=-1)
        # print(pairwise_features.shape)
        return pairwise_features, shapes
