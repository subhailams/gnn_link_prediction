...
def test_with_preds(
    ...
    """
    Evaluate the model and calculate metrics: AUC, MRR, Hits, and AP.
    Args:
        model: The trained model.
        score_func: Scoring function for predictions.
        data: Dataset containing test samples.
        evaluator_hit: Evaluator for Hits@k metrics.
        evaluator_mrr: Evaluator for MRR metrics (optional).
        batch_size: Batch size for evaluation.
        k_list: List of k values for Hits@k evaluation.
    
    Returns:
        metrics_results: Dictionary containing AUC, MRR, Hits, and AP metrics.
    """
    model.eval()
    score_func.eval()
    ...
    with torch.no_grad():
        if "citation" in data['dataset']:
            h = model.propagate()
            print("Getting Positive Predictions ...")
            pos_test_pred = test_edge_citation2(
                model, score_func, data['test_pos'], h, batch_size, test=True
            ).view(-1)
            print("Getting Negative Predictions ...")
            neg_test_pred = test_edge_citation2(
                model, score_func, data['test_pos'], h, batch_size, 
                mrr_mode=True, negative_data=data['test_neg'], test=True
            )
        else:
            print("Getting Positive Predictions ...")
            pos_test_pred = test_edge(
                model, score_func, data['test_pos'], batch_size, test_set=True
            ).squeeze(-1)
            print("Getting Negative Predictions ...")
            neg_test_pred = test_edge(
                model, score_func, data['test_neg'], batch_size, test_set=True
            ).squeeze(-1)

        # Convert predictions and labels for AUC/AP evaluation
        all_preds = torch.cat([pos_test_pred, neg_test_pred], dim=0).cpu().numpy()
        all_labels = torch.cat(
            [torch.ones_like(pos_test_pred), torch.zeros_like(neg_test_pred)], dim=0
        ).cpu().numpy()
        ...
        # AUC and AP
        metrics_results["AUC"] = roc_auc_score(all_labels, all_preds)
        metrics_results["AP"] = average_precision_score(all_labels, all_preds)
